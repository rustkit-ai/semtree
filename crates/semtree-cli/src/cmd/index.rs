...
pub async fn run(path: &Path, index_dir: &Path, config: &SemtreeConfig, full: bool) -> Result<()> {
    ...
    // Only treat a manifest as "existing" when one is actually on disk, so a
    // first-ever index does not report a phantom incompatibility.
    let has_prior_index = index_dir.join("manifest.json").exists();
    let existing = (!full && has_prior_index).then(|| FileManifest::load(index_dir));
